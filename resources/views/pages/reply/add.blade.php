...
@section('content')
    <div class="basic-container divided-container">
        <div id="write-message-wrapper">
            <h1 class="dc-title">Add reply</h1>
            <form action="{{route('reply.doAdd', $routeVariables)}}" method="POST" enctype="multipart/form-data" id="new-thread-form" onsubmit="return thread.validateAddingReply()">
                @if (count($errors) > 0)
                ...
                @endif
                @if(isset($responsePreview))
                    <div class="response-preview">
                        <p class="response-preview-header">Respond to message written by <span>{{$responsePreview->username}}</span>:</p>
                        <p class="response-preview-text">"{{$responsePreview->content}}"</p>
                    </div>
                @endif
